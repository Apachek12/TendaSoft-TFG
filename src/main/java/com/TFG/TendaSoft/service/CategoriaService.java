...
public class CategoriaService {
    ...
    public Categoria crearCategoria(Categoria categoria){
        if(categoria.getId() != null){
            throw new IllegalArgumentException("El ID debe estar vacío.");
        }
    ...
    public void eliminarCategoria(Integer id) {
        ...
        if (!productosAfectados.isEmpty()) {
            for (Producto p : productosAfectados) {
                p.setCategoria(categoriaPorDefecto);
            }
    ...
    private void validarDatosCategoria(Categoria categoria) {
        ...
        if (categoria.getOrden() == null || categoria.getOrden() < 0) {
            throw new IllegalArgumentException("El orden de visualización debe ser un número válido (0 o mayor).");
        }

        if (categoria.getCategoriaPadre() != null && categoria.getId() != null) {
            if (categoria.getId().equals(categoria.getCategoriaPadre().getId())) {
